...
async function getPageInAdmin<T>(url:URLS, page?: string) {
  try {
    const response = await adminFetch2(getUrl(url,`?page=${page || "1"}`), METHODS.GET);
...
async function getOneInAdmin<T>(url:URLS, id: string) {
  try {
    const response = await adminFetch2(getUrl(url, `/${id}`), METHODS.GET);
...
async function getOnePageByOneInAdmin<T>(url:URLS, id: string, page?: string) {
  try {
    const response = await adminFetch2(getUrl(url, `/${id}?page=${page || "1"}`), METHODS.GET);
    return response as Page<T>;
  } catch (e) {
    if(e instanceof Error) {
      if(e.message === "Unauthorized") throw new Error("로그인이 필요한 서비스입니다.");
...
async function deleteOne(url:URLS, id:string) {
  try {
    const response = await adminFetch2(getUrl(url, `/${id}`), METHODS.DELETE);
    return true;
  } catch (e) {
    console.log(e);
    if(e instanceof Error) {
...
async function getOneList(url:URLS, id?:string) {
  try {
    ...
    const response = await adminFetch2(getUrl(url, suffix), METHODS.GET);
...
async function addOrUpdate<T>(url:URLS, isUpdate: boolean, body:any) {
  try {
    const response = await adminFetch2(getUrl(url), isUpdate ? METHODS.PATCH : METHODS.POST, body);
    return response as T;
  } catch (e) {
    if(e instanceof Error) {
      if(e.message === "Unauthorized") throw new Error("로그인이 필요한 서비스입니다.");
