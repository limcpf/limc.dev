  ...
  return (
    <div className="flex flex-col justify-center mb-3">
      <div className="pre-title flex text-sm max-w-fit font-extralight text-gray-500 mb-1">
        <Link href={`/topic/${topic}`} className="topic mr-1 hover:underline">{topicName}</Link>
        {seriesName && <Link href={`/series/${series}`} className="series "> | <span className="hover:underline">{seriesName}</span></Link>}
      </div>
