...
export default function SiteList({id, page, getFunc, isAdmin}:{
    id?: string;
    page?: string;
    getFunc: getListFunc<Site>
    isAdmin?: boolean
}) {
    const [curPageSite, setCurPageSite]
        = useState<Page<Site>>();
                ...
                curPageSite
                    ? (
                        curPageSite.content.length > 0 ? (
                            curPageSite.content.map((site) => <SiteListItem site={site} isAdmin={isAdmin} />)
