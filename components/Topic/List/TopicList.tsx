...
export default function TopicList({id, page, getFunc, isAdmin}:{
    id?: string;
    page?: string;
    getFunc: getListFunc<Topic>
    isAdmin?: boolean
}) {
    const [curPageTopic, setCurPageTopic]
        = useState<Page<Topic>>();
                ...
                curPageTopic
                    ? (
                        curPageTopic.content.length > 0 ? (
                            curPageTopic.content.map((topic) => <TopicListItem topic={topic} isAdmin={isAdmin} />)
