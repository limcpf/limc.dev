...
import {getSeriesList, getSiteList, getTopicList, addPost, updatePost} from "@/libs/api/Admin.api";
import PostDto from "@/libs/dto/admin/PostDto";
import {useRouter} from "next/navigation";

export default function PostAddOrUpdate({ post }: {
    post?: Post
}) {
    ...
    useEffect(() => {
        if(isUpdate) {
            ...
            if(
                (hasWriting && hasWritingTime)
                && confirm(`${new Date(hasWritingTime).toISOString()}에 등록 실패한 게시글이 있습니다. 불러오시겠습니까?`)
            ) {
    ...
    const onSubmit: FormEventHandler<HTMLFormElement> = (evt) => {
        ...
        if(isUpdate) updatePost(postDto).then(addOrUpdateCallback).catch(addOrUpdateCatchCallback)
        else addPost(postDto).then(addOrUpdateCallback).catch(addOrUpdateCatchCallback)
    };

    const inputStyle = "bg-gray-50 border border-gray-300 text-gray-700 text-sm rounded-lg block w-full p-2.5";
    return (
        <div className="w-full py-3">
            <div className="text-center text-2xl font-bold mb-5">게시글 {isUpdate ? "수정" : "작성"}</div>
